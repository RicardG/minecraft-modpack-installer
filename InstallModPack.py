...
def main():
    ...
    if not noforge:
        ...
        print("Running forge installer (It should come up as a separate window)")
    ...
    if not noprofile:
        ...
        # the profile is where we expect it to be, make a new profile
        # (or overwrite a previous one if the user is stupid enough to name their profile after a mod pack)
    ...
    # now to 'install' the modpack by copying over the mods folder and any folder specified in the overrides
    print("\nInstalling mods")
...
# given an url, will try and download it, returns (error, requestobject/string)
def downloadURL(url):
    if url is None:
